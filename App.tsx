...
export default function App() {
  ...
  const generateProgressReport = (tasks: Task[]) => {
    ...
    stages.forEach((stage, stageIndex) => {
      ...
      const stageTasks = tasks.filter(
        (task) =>
          task.stages[stage as keyof typeof task.stages],
      );
      ...
      if (stageTasks.length === 0) {
        ...
        stageTasks.forEach((task) => {
          const stageData =
            task.stages[stage as keyof typeof task.stages];
          if (stageData) {
            const completed = stageData.filter(
    ...
    const totalCompleted = tasks.reduce((acc, task) => {
      return (
        acc +
        Object.values(task.stages).reduce((stageAcc, stage) => {
          return (
            stageAcc +
            stage.filter((checkbox) => checkbox.completed)
              .length
          );
        }, 0)
      );
    ...
    return {
      reportDiv,
      fileName: `${currentMonth}_${currentYear}_WorkflowTracker_Progress`,
    };
  };
  ...
  const handleSaveAndRefresh = async () => {
    ...
    try {
      ...
      const { reportDiv, fileName } =
        generateProgressReport(tasks);
      ...
      const resetTasks = INITIAL_TASKS.map((task) => ({
        ...task,
        stages: {
          stage1: task.stages.stage1.map((checkbox) => ({
            ...checkbox,
            completed: false,
          })),
          stage2: task.stages.stage2.map((checkbox) => ({
            ...checkbox,
            completed: false,
          })),
          stage3: task.stages.stage3.map((checkbox) => ({
            ...checkbox,
            completed: false,
          })),
          stage4: task.stages.stage4.map((checkbox) => ({
            ...checkbox,
            completed: false,
          })),
        },
      ...
      try {
        ...
        console.error(
          "Error resetting progress on server:",
          error,
        );
      }
      ...
      alert(
        `Progress saved as ${fileName}.pdf and all tasks have been reset!`,
      );
    } catch (error: any) {
      ...
      let errorMessage =
        "Error generating progress report. Please try again.";
      if (
        error.message &&
        error.message.includes("color function")
      ) {
        errorMessage =
          "PDF generation failed due to unsupported color formats. The issue has been logged for fixing.";
      } else if (
        error.message &&
        error.message.includes("oklch")
      ) {
        errorMessage =
          "Color format error detected. Please refresh the page and try again.";
      }
        ...
        <div className="flex items-center gap-3 relative z-10">
          ...
          <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center shadow-lg">
            ...
            >
              ...
              <path
                d="M12 5c-1.1 0-2 0.9-2 2s0.9 2 2 2s2-0.9 2-2s-0.9-2-2-2z"
                opacity="0.7"
              />
            </svg>
          </div>
            ...
            <div className="settings-dropdown">
              ...
              >
                <div className="flex items-center gap-1">
                  ...
                  <RefreshCw
                    className={`w-3 h-3 ${isGeneratingReport ? "animate-spin" : ""}`}
                  />
                </div>
                {isGeneratingReport
                  ? "Generating..."
